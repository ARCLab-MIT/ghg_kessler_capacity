...
The atmospheric mass density profile is located in data/dens_forecast_ssp.mat.
...
def main():
    ...
    fname1 = 'data/alt_ref_5y_30_10.mat'
    fname2 = 'data/alt_ref_25y_30_30.mat'
    if os.path.isfile(fname1) and os.path.isfile(fname2):
        alt_5y = sio.loadmat(fname1)['alt_5y']
        alt_25y = sio.loadmat(fname2)['alt_5y']
    else:
        # load processed file with time and altitude-resolved density profiles for each of the SSPs. 
        processed_file = 'data/dens_forecast_ssp.mat'
        data = sio.loadmat(processed_file)
        # Format: YEAR, ALT
        ...
        dt = 100*day_s # time step in seconds
        ...
        for k in range(len(t_deorbit_target)): 
            ...
            for j in range(4):#len(ssp_array_names)):
            ...
            # with open('data/alt_ref_'+str(int(t_deorbit_target[k]/year_s))+'y_'+str(round(abs(t0_vec[1]-t0_vec[0])*365.25)) + '_' + str(round(dt/day_s)) + '.pkl', 'wb') as f:
            #     pickle.dump(alt_ref, f)
                
            # save using savemat
            sio.savemat('data/alt_ref_'+str(int(t_deorbit_target[k]/year_s))+'y_'+str(round(abs(t0_vec[1]-t0_vec[0])*365.25)) + '_' + str(round(dt/day_s)) + '.mat', {labels[k]: alt_ref})
    ...
    # make a whitesmoke box in the background of the plot over all y and from x = 2000-2024
...
def t_to_deorbit(alt,t0,  B, mu, r_e, dt, ssp_dict, ssp):
    ...
    for i in range(int(max_time/t_step)): # 100 years max
...
def den(alt, t,  ssp_dict, ssp):
    ...
    alt_closest = np.argmin(np.abs(ssp_dict['alt_rs'][0]-alt))
    year_closest = np.argmin(np.abs(ssp_dict['year_rs'][0]-t))
